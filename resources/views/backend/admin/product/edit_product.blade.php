            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{route('product.update',[$product->id])}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            <label for="fname"
                                class="col-sm-3 text-end control-label col-form-label">Product Owner <span class="red">*</span></label>
                            <div class="col-sm-6">
                    ...
                    <div class="form-group row mb-0">
                        <div class="col-md-9 offset-md-3"> 
                            <div style="float: right;margin-top:10px;">
                                <a href="javascript:" class="btn btn-success" onclick="confirm('Are you shure want to approve this product') && approveit({{$product->id}});">Approve</a>
                                <a href="javascript:" class="btn btn-warning" onclick="confirm('Are you shure, this product information is not properly written, so need to edit') && needtoedit({{$product->id}})">Need to Edit</a>
                            </div>
                        </div>
                    </div>
...
<script>
    function approveit(id)
    {
        $.ajax({
            type:'GET',
            url: '/member_product/approve/'+id,
            dataType:'json',
            success:function(data){
                if(data == 'success')
                {
                    window.location.href = '/product/view_productlist_waiting_for_approve';
                }
            }
        })
    }
    function needtoedit(id)
    {
        $.ajax({
            type:'GET',
            url: '/member_product/needtoedit/'+id,
            dataType:'json',
            success:function(data){
                if(data == 'success')
                {
